...
os.environ["OPENAI_API_KEY"] = "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"


# define a function to calculate nth fibonicci number
def fib(n):
    if n <= 1:
        return n
    else:
...
#retrun direct means that output of the function will be returned
...
tools = [
    ...
    description = "use when you want to sort the string alphabetially",
    #return_direct =True
    ),
...
#verbose =Ture is to see the conversation's thought process

st.header(":blue[Langchain chabot with Tools and Agents with memory]") 
#short term memory, no embedded Vectorstore
user_input = st.text_input("You:")
#initilaize the memory buffer
...
if  st.button("Submit"):
    ...
    # add converstaions history to the memory buffer
